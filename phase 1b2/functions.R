...
sim_data1 = function(N2,lambda,seeeed,p_ORR,p_TOX,Sigma){ 
  
  colname =c("Time","Group","orr","tox");
  arm0 = as.data.frame(matrix(0,N2,length(colname))); colnames(arm0)=colname;
  arm1=arm0; arm2=arm0
  ...
  for (dose in 1:3) {
    set.seed(dose+seeeed)
    Z <- mvrnorm(N2, mu = rep(0, 3), Sigma = Sigma)
