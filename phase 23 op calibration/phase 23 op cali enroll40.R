...
for (HH in c("H1")){ # ###type I error: H0 ; power : H1
  ...
  for (Iter in 1:M){
    ...
    remd = N2%%40; quot= N2%/%40#remainder and the quotient :
    data_s$dif[which(data_s$Group==0& data_s$Stage==2)]= sample(c(13+rep(1:quot-1,each=40),+rep(13+quot,remd)),N2)
    data_s$dif[which(data_s$Group==1& data_s$Stage==2)]= sample(c(13+rep(1:quot-1,each=40),+rep(13+quot,remd)),N2)
...
output_suc <- output %>% filter(decision != 0)
...
output_fail$ess[output_fail$`FA time` != 0] = 20*2+N1*3+N2*2
...
save.image(file = paste0("T1S1_N2_",N2,"_HR_",HR_string,"_40",".RData"))